...
\ref{tab:einzelne_spulen} entnommen werden. Der Theoriewert wird nach \eqref{eqn:spuleungenaehert} berechnet.
Der Ursprung wurde dabei an den linken Rand der Spule gelegt, wobei berücksichtigt werden
muss, dass die Windungen erst $\SI{1,5}{\centi\metre}$ weiter beginnen.
...
Werte aufgenommen.
Der Theoriewert für die kurze Spule wird ebenfalls mit \eqref{eqn:spuleungenaehert} berechnet.
...
Der theoretisch maximal zu erwartende Wert ergibt sich gemäß \eqref{eqn:helmholz} zu $B_{\symup{theo}}=\SI{7,19}{\milli\tesla}$. In
...
erwartende Wert ergibt sich hier ebenfalls nach \eqref{eqn:helmholz} zu $B_{\symup{theo}}=\SI{4,32}{\milli\tesla}$. Die Messwerte
