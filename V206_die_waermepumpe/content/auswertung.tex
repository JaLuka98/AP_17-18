...
die Drücke $p_{\text{warm}}$ und $p_{\text{kalt}}$ und die Leistungsaufnahme $P$ des Kompressors
...
$p_{\symup{kalt}}$ wird noch der Umgebungsdruck von $\SI{1e5}{\pascal}$ hinzuaddiert.
...
Reservoir sind in Tabelle \ref{tab:parameter} zu finden.
...
Fehlerfortpflanzung (vgl. Gleichung \eqref{eqn:gaussfehler}) in der Form
\begin{equation}
...
\end{equation}
berechnen.
Der Fehler bei der realen Güteziffer ergibt sich dabei gemäß Gleichung \eqref{eqn:gaussfehler}
durch die Gauß'sche Fehlerfortpflanzung
\begin{equation}
...
Dichlordifluormethan mithilfe einer Dampfdruck-Kurve bestimmt\footnote{Die
theoretische Grundlage hierfür kann in \cite{dampfdruck} nachgelesen
...
\end{equation*}
bestimmt. Mit der Gauß'schen Fehlerfortpflanzung nach Gleichung \eqref{eqn:gaussfehler} ergibt sich
...
\eqref{eqn:kompressorleistung} des Kompressors zu den vier ausgewählten Zeitpunkten
