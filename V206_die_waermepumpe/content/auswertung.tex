...
Der Massendurchsatz wird über die Gleichungen \eqref{eqn:massendurchsatz} und \eqref{eqn:waermekaltprozeit}
bestimmt. Es wird zunächst die molare Verdampfungswärme $L_n$ des Transportgases
...
werden.} und daraus die auf die Masse bezogene Verdampfungswärme $L_m$ errechnet.
...
\end{equation}
in den Parametern $A_L$ und $B_L$ und durchgeführt. Dabei bezeichnet $R$ die universelle Gaskonstante.
...
\begin{table}
	...
	\begin{tabular}{cccc}
		\toprule
		$t/$s & $\rho/(g \cdot l^{-1}$) & $N_\text{mech} / [\mathrm{W}]$ & $\eta$\\
		\midrule
